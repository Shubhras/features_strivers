...
@section('content')
	...
	<div class="main-container">
					...
					<div class="inner inner-box items-details-wrapper pb-0">
						...
						<span class="info-row">
							...
							<span class="category"{!! (config('lang.direction')=='rtl') ? ' dir="rtl"' : '' !!}>
								<i class="fas fa-folder"></i> {{ (!empty($post->category->parent)) ? $post->category->parent->name : $post->category->name }}
							</span>&nbsp;
							<span class="item-location"{!! (config('lang.direction')=='rtl') ? ' dir="rtl"' : '' !!}>
								<i class="fas fa-map-marker-alt"></i> {{ $post->city->name }}
							</span>&nbsp;
							<span class="category"{!! (config('lang.direction')=='rtl') ? ' dir="rtl"' : '' !!}>
								<i class="fas fa-eye"></i> {{ \App\Helpers\Number::short($post->visits) }} {{ trans_choice('global.count_views', getPlural($post->visits), [], config('app.locale')) }}
							</span>
						</span>
									...
									<div class="row pb-3">
										<div class="items-details-info col-md-12 col-sm-12 col-12 enable-long-words from-wysiwyg">
											...
											<div class="row">
												...
												<div class="col-md-6 col-sm-6 col-6">
													<h4 class="fw-normal p-0">
														<span class="fw-bold"><i class="fas fa-map-marker-alt"></i> {{ t('location') }}: </span>
														<span>
											...
											@if (!empty($post->tags))
												<div class="row mt-3">
													<div class="col-12">
														<h4 class="p-0 my-3"><i class="fas fa-tags"></i> {{ t('Tags') }}:</h4>
									...
									@if (auth()->check())
										@if (auth()->user()->id == $post->user_id)
											<a href="{{ \App\Helpers\UrlGen::editPost($post) }}" class="btn btn-default btn-block">
												<i class="fas fa-edit"></i> {{ t('Update the Details') }}
											</a>
		...
		@if (config('settings.single.similar_posts') == '1' || config('settings.single.similar_posts') == '2')
			<?php $widgetType = (config('settings.single.similar_posts_in_carousel') ? 'carousel' : 'normal') ?>
			@includeFirst([
