...
<script>
    ...
    {
        /* Send an ajax update request */
        $(document).on('click', '.ajax-request-aproove', function(e)
        {
            e.preventDefault(); /* prevents the submit or reload */
            var confirmation = confirm("<?php echo trans('admin.confirm_this_action'); ?>");
            
            if (confirmation) {
                saveAjaxRequestAproove(siteUrl, this);
            }
        });
    });
    ...
    function saveAjaxRequestAproove(siteUrl, el)
    {
        if (isDemoDomainJs()) {
            return false;
        }
        ...
        $.ajax({
            method: 'POST',
            // dataType: 'application/json',
            url: siteUrl + '/<?php echo admin_uri(); ?>/users_aproove',
            context: this,
            data: {
                'primaryKey': dataId,
                '_token': _token
