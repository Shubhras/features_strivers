...
trait MailTrait
{
	...
	public function mailUpdating($setting, $original)
	{
		...
		if ($validateDriverParameters) {
			...
			try {
				...
				$message = cleanAddSlashes($e->getMessage());
	...
	private function updateMailConfig($setting)
	{
		...
		// Mandrill
		if (config('mail.default') == 'mandrill') {
			config()->set('services.mandrill.secret', $setting->value['mandrill_secret'] ?? null);
			config()->set('mail.mailers.smtp.host', $setting->value['mandrill_host'] ?? null);
			config()->set('mail.mailers.smtp.port', $setting->value['mandrill_port'] ?? null);
			config()->set('mail.mailers.smtp.encryption', $setting->value['mandrill_encryption'] ?? null);
			config()->set('mail.mailers.smtp.username', $setting->value['mandrill_username'] ?? null);
			config()->set('mail.mailers.smtp.password', $setting->value['mandrill_password'] ?? null);
			config()->set('mail.from.address', $setting->value['mandrill_email_sender'] ?? null);
		}
