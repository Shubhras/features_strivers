	...
	public function applyFrontSettings()
	{
		...
		*/
		...
		if (!empty($display) && isset($typeOfDisplay[$display])) {
			...
			Cookie::queue('display_mode', $display, $this->cookieExpiration);
		} else {
			if (request()->hasCookie('display_mode')) {
				$display = request()->cookie('display_mode');
			}
		}
		if (!empty($display) && isset($typeOfDisplay[$display])) {
			config(['settings.listing.display_mode' => $typeOfDisplay[$display]]);
		}
