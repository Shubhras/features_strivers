...
use App\Models\User;
use App\Models\Strivre;
use Illuminate\Support\Facades\DB;
use Illuminate\Support\Facades\Hash;
use Illuminate\Support\Str;
use Larapen\Admin\app\Http\Controllers\PanelController;

class StrivreController extends PanelController
{
...
// 	$datane= 	DB::table('user_strivre')->select('user_strivre.*')->get();
// print_r($datane);die;
		...
		// 	$usersIds = [];
		// 	try {
		// 		$users = Strivre::withoutGlobalScopes([VerifiedScope::class])->role('super-admin')->get(['id', 'created_at']);
		// 		if ($users->count() > 0) {
	...
	public function account()
	{
		...
		$this->xPanel->addField([
			...
			'label'             => trans('admin.subscription_plans'),
			'type'              => 'text',
			'attributes'        => [
				'placeholder' => trans('admin.subscription_plans'),
			],
			'wrapperAttributes' => [
				'class' => 'col-md-6',
			],
