...
class Country
{
	...
	public function validateTheCountry()
	{
		...
		if (
			...
			|| (isFromAdminPanel() && in_array(request()->segment(1), ['captcha']))
	...
	public static function getCountryInfo($countryCode)
	{
		...
		$tzColumnIsAvailable = Cache::remember($cacheId, $cacheExpiration, function () use ($countryCode) {
			$tzColumnIsAvailable = Schema::hasColumn((new CountryModel())->getTable(), 'time_zone');
			
			return $tzColumnIsAvailable;
		});
	...
	public static function getCountries()
	{
		...
		try {
			$countries = Cache::remember('countries.with.continent.currency', self::$cacheExpiration, function () {
				$countries = CountryModel::active()->with(['continent', 'currency'])->orderBy('name')->get();
