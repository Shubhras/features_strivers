 ...
 * @return bool
 */
function doesCountriesPageCanBeHomepage()
{
	return (
 ...
 * @return bool
 */
function doesCountriesPageCanBeLinkedToTheHomepage()
{
	return (
		file_exists(storage_path('framework/plugins/domainmapping'))
		&& (env('DM_COUNTRIES_PAGE_AS_HOMEPAGE') == true)
 ...
 * @return string
 */
function getUploadedFileMimeType($value)
{
	$mimeType = null;
 ...
 */
function tagCleaner($tagString, $forceArrayReturn = false)
{
 ...
 * @return \Illuminate\Contracts\Foundation\Application|\Illuminate\Contracts\Routing\UrlGenerator|string
 */
function privateFileUrl($filePath, $diskName = 'private')
{
...
/**
 * @param $filePath
 * @param string $preConfigSize
 * @param array $attr
 * @return string
 */
function imgTag($filePath, $preConfigSize = 'big', $attr = [])
{
	...
	if (config('settings.optimization.webp_format')) {
		...
		if (!\Illuminate\Support\Str::endsWith($srcWebp, '.webp')) {
 ...
 * @return string
 */
function imgUrl($filePath, $preConfigSize = 'big', $webpFormat = false)
{
 ...
 * @return string
 */
function resize($disk, $filePath, $preConfigSize = 'big', $webpFormat = false)
{
	// Image Quality
 ...
 * @return string
 */
function resizeWebp($disk, $filePath, $type = 'big')
{
 ...
 */
function replaceGlobalPatterns($string, $removeUnmatchedPatterns = true)
{
	...
	if (\Illuminate\Support\Str::contains($string, '{count.ads}')) {
		try {
	...
	if (\Illuminate\Support\Str::contains($string, '{count.users}')) {
		try {
...
/**
 * Remove all unmatched variables patterns (eg. {foo}) from a string
 *
 * @param $string
 * @return string
 */
function removeUnmatchedPatterns($string)
{
	if (!is_string($string)) return $string;
 ...
 *
 */
function arrayItemsAreEmpty($array)
{
	if (empty($array)) {
		return true;
	}
...
function genEmailContactBtn($post = null, $btnBlock = false, $iconOnly = false)
{
	...
	if (!isset($post->email) || empty($post->email)) {
		if ($iconOnly) {
			$out = '<i class="fas fa-envelope" style="color: #dadada"></i>';
		}
	...
	if ($iconOnly) {
		...
		$out .= '<i class="fas fa-envelope" data-bs-toggle="tooltip" title="' . t('Send a message') . '"></i>';
		$out .= '</a>';
	} else {
		if ($btnBlock) {
		...
		$out .= '<a href="' . $btnLink . '" class="btn btn-default' . $btnClass . '" data-bs-toggle="modal">';
