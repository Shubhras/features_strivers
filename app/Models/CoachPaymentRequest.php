...
class CoachPaymentRequest extends BaseModel
{
	use Crud, HasFactory;
	 ...
	 * @var string
	 */
	protected $table = 'coach_payment_request';
	 ...
	 * @var array
	 */
	protected $fillable = ['created_at','coach_id', 'email','payment', 'active'];
	...
	*/
	protected static function boot()
	{
		...
		CoachPaymentRequest::observe(PageObserver::class);
    ...
    public function getPackageNameHtml()
	{
		...
		if (!empty($this->coach_payment_request)) {
			$packageUrl = admin_url('coach_payment_request/' . $this->id . '/edit');
			
			$out = '';
			$out .= '<a href="' . $packageUrl . '">';
			$out .= $this->coach_payment_request->id;
			$out .= '</a>';
			$out .= ' (' . $this->coach_payment_request->email . ')';
		}
